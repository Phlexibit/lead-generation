...
const GoalAssignment = require("../goal_assignment/models");
...
const deleteGoal = async (req, res) => {
  try {
    ...
    await GoalAssignment.deleteMany({ goal: goalId });
    await Goal.findByIdAndDelete(goalId);

    res.json({
      success: true,
      message: "Goal and associated assignments deleted successfully",
    });
  } catch (error) {
    res.status(500).json({
      success: false,
